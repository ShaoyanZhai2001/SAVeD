...
"""
Runtime safety toggles for TITAN RTX (sm_75):
- Force SDPA to math kernels only (disable flash/mem-efficient) to avoid any BF16-specific kernels.
- Monkeypatch torch.cuda.is_bf16_supported to False so any downstream auto-logic never selects BF16.
"""
try:
    if hasattr(torch, 'cuda') and torch.cuda.is_available():
        try:
            from torch.backends.cuda import sdp_kernel
            # Only use math kernels; disable flash and mem-efficient paths.
            sdp_kernel(enable_flash=False, enable_mem_efficient=False, enable_math=True)
        except Exception:
            pass
        try:
            # Guard all callers from ever thinking BF16 is supported on sm_75
            torch.cuda.is_bf16_supported = lambda: False  # type: ignore[attr-defined]
        except Exception:
            pass
except Exception:
    pass
...
def parse_args():
    ...
    optim_args.add_argument(
        '--vit_lr', default=4e-5, type=float, help='learning rate.')
    optim_args.add_argument(
        '--connector_lr', default=4e-5, type=float, help='learning rate.')
    optim_args.add_argument(
...
class LazyInternVL2Dataset(BaseOrigDataset):
    ...
    def video_get_item(self, data_item, pack_data=False):
        ...
        if not os.path.exists(video_path):
            ...
            logger.warning(f"  self.root = {self.root}")
            logger.warning(f"  video_file = {video_file}")
            logger.warning(f"  Expected path = {video_path}")
            raise FileNotFoundError(f"Video file does not exist: {video_path}")

        if 'start' in data_item:
            clip = [data_item['start'], data_item['end']]
        else:
...
def packing_collate(features, pack_batch=True, pad_id=0, sp_size=1):
    ...
    # Ensure vision inputs are float16
    if isinstance(pixel_values, torch.Tensor) and pixel_values.dtype != torch.float16:
...
def build_llava_model(args, dtype=torch.float32, device='cpu'):
    with torch.device(device):
        with LoadWoInit():
            ...
            if hasattr(_cfg, 'torch_dtype'):
                _cfg.torch_dtype = torch.float16
            if hasattr(_cfg, 'vision_config') and hasattr(_cfg.vision_config, 'torch_dtype'):
                _cfg.vision_config.torch_dtype = torch.float16
            # Explicitly disable FlashAttention in the vision stack at config level
            ...
            if hasattr(_cfg, 'vision_config') and hasattr(_cfg.vision_config, 'attn_implementation'):
                _cfg.vision_config.attn_implementation = 'sdpa'
            vision_model = InternVisionModel(_cfg.vision_config)
            # Avoid loading checkpoint weights when building a meta model; construct from config instead
            if str(device) == 'meta':
                internvl = AutoModel.from_config(
                    _cfg,
                ...
                internvl = AutoModel.from_pretrained(
                    args.model,
                    vision_model=vision_model,
                    use_flash_attn=False,  # Disabled for TITAN RTX (compute capability 7.5)
                    trust_remote_code=True,
                    torch_dtype=torch.float16,
                    low_cpu_mem_usage=False,  # load directly without meta-parameter juggling
                )
            # Force disable FlashAttention and bf16 at runtime regardless of config/checkpoint
            if hasattr(internvl, 'vision_model'):
                internvl.vision_model.use_flash_attn = False
                if hasattr(internvl.vision_model, 'attn_implementation'):
                    internvl.vision_model.attn_implementation = 'sdpa'
                if hasattr(internvl.vision_model, 'torch_dtype'):
                    internvl.vision_model.torch_dtype = torch.float16
                # Recursively patch all encoder layers and attention modules
                encoder = getattr(internvl.vision_model, 'encoder', None)
                if encoder and hasattr(encoder, 'layers'):
                    for layer in encoder.layers:
                        if hasattr(layer, 'use_flash_attn'):
                            layer.use_flash_attn = False
                        if hasattr(layer, 'attn_implementation'):
                            layer.attn_implementation = 'sdpa'
                        if hasattr(layer, 'torch_dtype'):
                            layer.torch_dtype = torch.float16
                        attn = getattr(layer, 'attn', None)
                        if attn and hasattr(attn, 'use_flash_attn'):
                            attn.use_flash_attn = False
                        if attn and hasattr(attn, 'attn_implementation'):
                            attn.attn_implementation = 'sdpa'
                        if attn and hasattr(attn, 'torch_dtype'):
                            attn.torch_dtype = torch.float16
            if hasattr(internvl, 'config') and hasattr(internvl.config, 'vision_config'):
                internvl.config.vision_config.use_flash_attn = False
                internvl.config.vision_config.torch_dtype = torch.float16
                internvl.config.vision_config.attn_implementation = 'sdpa'
            tokenizer = AutoTokenizer.from_pretrained(args.model, trust_remote_code=True)
            img_context_token_id = tokenizer.convert_tokens_to_ids('<IMG_CONTEXT>')
            internvl.img_context_token_id = img_context_token_id

        internvl.to(torch.float16)

        # Safety: ensure no residual BF16 params or buffers remain
        with torch.no_grad():
            for _, param in internvl.named_parameters(recurse=True):
                if isinstance(param, torch.nn.Parameter) and param.dtype == torch.bfloat16:
                    param.data = param.data.to(torch.float16)
            for _, buf in internvl.named_buffers(recurse=True):
                if isinstance(buf, torch.Tensor) and buf.dtype == torch.bfloat16:
...
def llava_train(args):
    ...
    # Select compute dtype: prefer bf16 only if supported; otherwise use fp16 on Turing (sm_75)
    if args.dtype == 'auto':
        dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
        args.dtype = 'bf16' if dtype is torch.bfloat16 else 'fp16'
    ...
    requried_grad_params_vit = []
    requried_grad_params_connector = []
    requried_grad_params_llm = []
    ...
    for step in range(start_step, total_steps):
        ...
        for _ in range(per_step_iters):
            ...
            # print(f"num_tokens={num_tokens}, num_img_tokens={num_img_tokens}, num_imgs={num_imgs}")
        ...
        # WORKAROUND: Disable checkpoints during training due to NCCL backend incompatibility
        # with allgather_into_tensor_coalesced in FSDP zero3 state dict gathering.
        # Only save at the very end (after training loop completes).
    ...
    if dist.get_rank() == 0:
        logger.info("[Checkpoint] Saving final model checkpoint...")
    try:
        save_ckpt(args, total_steps, total_steps, fsdp_model, rank0_model, warmup_scheduler, cosine_scheduler,
                  optimizer, max_keep_ckpts, save_hf_ckpt_names, save_pt_ckpt_names, tokenizer, processor)
    except Exception as e:
        if dist.get_rank() == 0:
            logger.warning(f"[Checkpoint] Failed to save checkpoint with FSDP state dict: {e}")
            logger.info("[Checkpoint] Attempting alternative save using rank0 model only...")
        # Fallback: save only rank0 model without gathering full FSDP state
        if dist.get_rank() == 0:
            hf_dir = os.path.join(args.work_dir, 'final-model')
            rank0_model.save_pretrained(hf_dir)
            if tokenizer is not None:
                tokenizer.save_pretrained(hf_dir)
            if processor is not None:
                processor.save_pretrained(hf_dir)
            logger.info(f"[Checkpoint] Saved model to {hf_dir}")
    # ------------------------    Training  End  ---------------------------- #
